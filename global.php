...
$query = "SELECT m.content, m.sent_at, a.name, a.emoji, a.is_admin FROM global_chat m JOIN accounts a ON m.user_id = a.user_id WHERE m.is_deleted = false AND m.chat_id = 0 ORDER BY m.sent_at ASC LIMIT 100";
...
<body class="bg-black text-white flex flex-col h-screen m-0 p-0 overflow-hidden">
    <header class="mt-5 shrink-0 text-center">
        <h1 class="text-4xl font-bold">great<span class="text-[#6674b2]">name</span>.net</h1>
    </header>

    <main class="flex flex-col flex-1 items-center w-full px-4 min-h-0">
        <div class="text-center mb-2 shrink-0">
            <h2 class="text-3xl font-bold mt-6">Global</h2>
        </div>
        <div class="flex-1 min-h-0 w-full mt-2 border border-zinc-800 rounded-t-xl overflow-y-auto p-4 flex flex-col gap-3" id="chatBox">
        ...
        <form action="message.php" method="POST" class="w-full shrink-0 flex gap-2 p-3 bg-zinc-900 border-x border-b border-zinc-800 rounded-b-xl">
    ...
    <footer class="mb-5 mt-5 w-full shrink-0 text-center">
